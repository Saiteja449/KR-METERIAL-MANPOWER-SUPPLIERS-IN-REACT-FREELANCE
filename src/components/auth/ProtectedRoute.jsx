      ...
      <div className="min-h-screen pt-28 pb-16 px-4 flex items-center justify-center bg-slate-light">
        <div className="max-w-md w-full bg-white p-8 rounded-sm shadow-xl border-t-4 border-amber text-center">
          ...
          <p className="text-gray-600 text-sm leading-relaxed mb-6">
            Your candidate dashboard is currently locked. Access will be unlocked automatically once your registration fee payment is verified.
          </p>
          {appId && (
          ...
          <div className="space-y-3">
            {appId && (
              <Link
                to={`/payment/${appId}`}
                className="w-full inline-flex items-center justify-center gap-2 bg-[#5f259f] hover:bg-[#4d1d82] text-white font-bold py-3 px-6 rounded-sm transition-all shadow-md"
              >
                <CreditCard size={18} />
                <span>Complete UPI / QR Payment</span>
              </Link>
            )}
            <Link
              to="/"
